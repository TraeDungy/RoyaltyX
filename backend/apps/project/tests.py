...
class ProjectUserListViewTests(TestCase):
    ...
    def test_add_user_without_role_uses_default(self):
        ...
        self.assertEqual(response.data["role"], ProjectUser.PROJECT_USER_ROLE_PRODUCER)  # Default role producer
    ...
    def test_project_field_automatically_set_from_currently_selected_project(self):
        """Test that project field is automatically set from user's currently selected project"""
        self.client.force_authenticate(user=self.owner)
...
class ProjectUserViewDeleteTests(TestCase):
    def setUp(self):
        ...
        self.project = Project.objects.create(
            name="Test Project",
            description="Test project for deletion tests"
        )
    ...
    def test_successful_delete_project_user(self):
        ...
        delete_url = reverse("project-user", kwargs={"id": self.project_user_to_delete.id})
