...
def fetch_youtube_videos(source_id=None):
    ...
    for source in sources:
        ...
        if not source.channel_id:
            logger.warning(
                "No channel_id set for source %s, skipping video fetch",
                source.id,
            )
            continue
...
def fetch_youtube_stats(source_id=None):
    ...
    for source in sources:
        ...
        if not source.channel_id:
            logger.warning(
                "No channel_id set for source %s, skipping stats fetch",
                source.id,
            )
            continue
        ...
        for product in products:
            ...
            if rows:
                ...
                logger.info("Views: %s", current_view_count)
            else:
                logger.info("No rows returned in stats.")
